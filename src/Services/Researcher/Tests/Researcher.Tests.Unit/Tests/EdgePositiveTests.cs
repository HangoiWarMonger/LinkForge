...
/// <summary>
/// Набор позитивных юнит-тестов для проверки создания и обновления сущности Edge.
/// </summary>
public class EdgePositiveTests
{
    /// <summary>
    /// Вспомогательный метод для создания валидного узла Node с опциональным идентификатором.
    /// </summary>
    ...
    /// <summary>
    /// Проверяет, что конструктор создаёт объект Edge и корректно прикрепляет его к узлам-источнику и приёмнику.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Проверяет, что метод Update корректно обновляет свойства Edge и перенаправляет связи между узлами.
    /// </summary>
    [Fact]
